...
class VAE(nn.Module):
    def __init__(self, input_shape, encoder_arch, num_latents, generator_arch, num_speakers):
    ...
    def build_module(self):
        ...
        x = torch.zeros((self.input_shape))
        self.encoder = EncoderVAE(input_shape=self.input_shape,
                            kernel_sizes=self.encoder_arch.kernel_sizes,
                            strides=self.encoder_arch.strides,
                            num_residual_channels=self.encoder_arch.num_residual_channels,
    ...
    def forward(self, input, speaker):
        ...
        z = self.sample_latent(mu, log_var)
        x_hat = self.generator(z, speaker)
...
class EncoderVAE(Encoder):
    """
    Downsampling encoder with mu and sigma output for VAE.
    """
    def __init__(self, input_shape, kernel_sizes, strides, num_residual_channels, latent_dim):
        super(EncoderVAE, self).__init__(input_shape, kernel_sizes, strides, num_residual_channels, latent_dim)
    ...
    def build_module(self):
        ...
        # Final convolution for mu and sigma
        mean_latent_conv = nn.Conv1d(in_channels=x.shape[1],
                                     out_channels=self.latent_dim,
                                     kernel_size=1,
                                     stride=1,
                                     padding=0)
        self.layer_dict['mean_latent_conv'] = mean_latent_conv

        mean_x = mean_latent_conv(x)

        logvar_latent_conv = nn.Conv1d(in_channels=x.shape[1],
                                       out_channels=self.latent_dim,
                                       kernel_size=1,
                                       stride=1,
                                       padding=0)
        self.layer_dict['logvar_latent_conv'] = logvar_latent_conv

        logvar_x = logvar_latent_conv(x)
        print(str(logvar_x.shape) + " and " + str(mean_x.shape))
    ...
    def forward(self, input):
        ...
        mean_out = self.layer_dict['mean_latent_conv'](out)
        logvar_out = self.layer_dict['logvar_latent_conv'](out)
